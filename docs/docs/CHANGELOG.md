...
### OWASP Core Rule Set snippets (beta)

Anubis is in the process of slowly porting over the [OWASP Core Rule Set](https://coreruleset.org/) in `(data)/security/coreruleset`. These rules are generated by the command `internal/cmd/coreruleset`, which builds them from the upstream `.data` pattern files.

Currently the following rules are supported:

| Rule                       | ID       | Description                                                                                                                                         |
| :------------------------- | :------- | :-------------------------------------------------------------------------------------------------------------------------------------------------- |
| `00-lfi-os-files.yaml`     | `930120` | Weighs requests that name operating system files such as `/etc/passwd` in query parameter values, query parameter names, cookies, or request paths. |
| `00-restricted-files.yaml` | `930130` | Weighs requests for restricted files such as `.htaccess`, `.git/config`, and framework configuration files.                                         |
| `00-ai-agent-configs.yaml` | `930140` | Weighs requests for AI coding assistant directories such as `.claude/`, `.cursor/`, and `.opencode/`.                                               |

Matching these rules makes requests gain five (5) [weight points](./admin/configuration/thresholds.mdx).

Please keep the following in mind:

- These snippets are beta and a work in progress. There is no backwards compatibility guarantee for them. Behaviour can and will change without notice.
- Enabling these snippets adds a lot of computation per request (1-5ms in my testing on my development box). This is a stress test of the policy engine. Do not enable them unless you have a lot of CPU to spare. This will be optimized in the future.
- These rules are designed with legacy deployments of PHP based web applications or other applications that dispatch data from the filesystem. Do **NOT** activate them if your web application does in-process routing of HTTP requests.
- Do **NOT** enable these rules on a git server. This has not been tested with git server traffic in mind. As currently written, this may cause the web frontend of git servers to receive Anubis challenge pages when browsing repositories instead of repository data.

If you run into issues with these rules, please disable them and file a bug.

### Fixed

- Add missing `yaml` struct tags to the `geoip` and `asns` fields of a bot policy rule. Without them, marshalling a rule to YAML emitted `geoip: null` and `asns: null` for rules that use neither.
