...
| `fcrdns`        | `fcrdns`              | Custom object for performing the forward-confirmed reverse DNS check.                                                                     | `fcrdns.check("\\.qwant\\.com$")`                            |
...
```ts
function check(pattern: string): bool;
```

`fcrdns.check` accepts a regex string and performs the same check as the `domain_regex` field. For more details, read [Reverse DNS based filtering](../policies#reverse-dns-based-filtering).

Example for search engine bots:
...
- name: qwantbot
  ...
  expression:
    all:
      - userAgent.matches("\\+https\\://help\\.qwant\\.com/bot/")
      - fcrdns.check("\\.qwant\\.com$")
  action: ALLOW
```
