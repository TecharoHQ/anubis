...
Caddy placeholders are supported for string settings during provisioning, so
you can use values such as `policy_file {env.ANUBIS_POLICY}` or
`ed25519_private_key_hex {file./run/anubis-ed25519.hex}`.
