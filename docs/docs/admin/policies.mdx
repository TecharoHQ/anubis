...
Rules that use `domain_regex` should also have a `user_agent_regex` or `expression` to avoid unnecessary DNS requests.
...
- name: qwantbot
  user_agent_regex: \+https\://help\.qwant\.com/bot/
  action: ALLOW
  # The reverse domain expected for legitimate requests (see https://help.qwant.com/bot/)
  domain_regex: \.qwant\.com$
```
