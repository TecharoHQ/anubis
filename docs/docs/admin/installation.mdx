...
| `OG_QUERY_DISTINCT`            | unset                   | A regex pattern to determine distinct cache keys for Open Graph tags. For example, `.*` will treat all URLs with different query parameters as distinct cache keys.                                                                                                                      |
