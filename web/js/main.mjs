...
const imageURL = (mood, cacheBuster, basePath) =>
  u(`${basePath}/.within.website/x/cmd/anubis/static/img/${mood}.webp`, { cacheBuster });
...
(async () => {
  ...
  const basePath = JSON.parse(document.getElementById('anubis_base_path').textContent);
  ...
  if (!window.isSecureContext) {
    ohNoes({
      ...
      imageSrc: imageURL("reject", anubisVersion, basePath),
    });
    return;
  }
  ...
  try {
    ...
    image.src = imageURL("happy", anubisVersion, basePath);
    progress.style.display = "none";
    ...
    if (userReadDetails) {
      ...
      function onDetailsExpand() {
        const redir = window.location.href;
        window.location.replace(
          u(`${basePath}/.within.website/x/cmd/anubis/api/pass-challenge`, {
            response: hash,
            nonce,
            redir,
            elapsedTime: t1 - t0
