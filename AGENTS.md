...
- **Confirmed macOS integration-test quirk**: On this Mac, `/bin/bash` is Bash 3.2 and does not support `shopt -s globstar`, while `npm run assets` runs `web/build.sh` and also expects `zstd`/`brotli`. `npm run test:integration` passed when Playwright was pre-started, `-playwright-runner=none` was injected through a temporary `go test -exec` wrapper, and temporary local shims handled the Bash/globstar and compression-tool gaps.
