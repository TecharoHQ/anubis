...
- **Confirmed Caddy global defaults**: `plugins/caddy` registers `anubis` as both a Caddyfile global option and handler directive. A global `{ anubis { ... } }` block supplies Caddyfile defaults only; handler-level `anubis` blocks merge those defaults before validation and still provision independent Anubis servers per route.
