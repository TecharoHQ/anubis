...
//
// TLS Support:
// Some tests use HTTPS with localhost certificates generated by mkcert. The certificates are expected to be
// located in internal/test/certs/localhost+2.pem and internal/test/certs/localhost+2-key.pem.
// These are automatically generated by the test setup if mkcert is available.
package test
...
func ensureTestCertificates(t *testing.T) {
	...
	certFile := "internal/test/certs/localhost+2.pem"
	keyFile := "internal/test/certs/localhost+2-key.pem"

	// Check if certificates exist
	if _, err := os.Stat(certFile); err == nil {
		if _, err := os.Stat(keyFile); err == nil {
			return // Certificates exist
		}
	}
	...
	t.Log("Generating test certificates with mkcert...")
	cmd := exec.Command("mkcert", "localhost", "127.0.0.1", "::1")
	cmd.Dir = "internal/test/certs"
	cmd.Stderr = os.Stderr
...
func spawnAnubisWithAdvancedOptions(t *testing.T, opts anubisTestOptions) string {
	...
	s, err := libanubis.New(libanubis.Options{
		...
		StripBasePrefix: opts.stripBasePrefix,
	})
	if err != nil {
	...
	if opts.useTLS {
		// Use mkcert certificates for a more realistic TLS setup
		certFile := "internal/test/certs/localhost+2.pem"
		keyFile := "internal/test/certs/localhost+2-key.pem"
		...
		ts = &httptest.Server{
			Listener: listener,
			Config:   &http.Server{Handler: s, TLSConfig: tlsConfig},
			TLS:      tlsConfig,
		}
		ts.StartTLS()
	} else {
